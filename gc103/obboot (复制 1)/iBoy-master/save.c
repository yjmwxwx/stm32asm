...
void loadstate(FILE *f)
{
	...
	un32 d;
	int irl = 2; //hw.cgb ? 8 : 2;
	int vrl = 2; //hw.cgb ? 4 : 2;
	int srl = mbc.ramsize << 1;

	
//	while ((inl(IBOY_COP_STATUS) & IBOY_COP_LINE_REQ)!=0); //SAVE PROBLEMS ?
...
void savestate(FILE *f)
{
	...
	memcpy(buf+oamofs, LCDP->oam.mem, sizeof LCDP->oam);
