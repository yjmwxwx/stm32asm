...
//struct mbc mbc; //FIXME is doch im asm drin - also nur extern..
struct rom rom;
struct ram ram;
...
void mem_updatemap()
{
	...
	*/
	...
	if (R_VBK & 1)
	{
		map[0x8] = LCDP->vbank2 - 0x8000;
		map[0x9] = LCDP->vbank2 - 0x8000;
	}
	else
	{
		map[0x8] = LCDP->vbank/*[R_VBK & 1]*/ - 0x8000;
		map[0x9] = LCDP->vbank/*[R_VBK & 1]*/ - 0x8000;
	...
	else mbc.rmap[0xA] = mbc.rmap[0xB] = NULL;
	...
	mbc.wmap[0xC] = ram.ibank[0] - 0xC000;
	n = R_SVBK & 0x07;
	map[0xD] = ram.ibank[n?n:1] - 0xD000;
	map[0xE] = ram.ibank[0] - 0xE000;
	map[0xF] = NULL;
...
#ifndef OLD_KERNEL
	outb(b, &(LCDP->mainpal[i]));
	outb(b, &(LCDP->pals[l][i]));


	i>>=1;
	index = i>>5;
	LCDP->palfetch[l][index&1] |= (1<<(i&0x1f));
#endif
	//updatepalette(i>>1);
}
...
case RI_BCPS:
		R_BCPS = b & 0xBF;
		#ifndef OLD_KERNEL
		R_BCPD = (LCDP->mainpal[b & 0x3F]);
		#endif
		break;
	case RI_OCPS:
		R_OCPS = b & 0xBF;
		#ifndef OLD_KERNEL
		R_OCPD = (LCDP->mainpal[64 + (b & 0x3F)]);
		#endif
		break;
	case RI_BCPD:
		R_BCPD = b;
...
/*	if(rombank_old != mbc.rombank) {
		if(cpu.pc.w[0] >= 0x4000 && cpu.pc.w[0] < 0x8000)
			printf("\nI cant believe this!\n");
...
int mem_write(int a, byte b)
{
	int n;
	byte ha = (a>>12) & 0xE;

	//fprintf(debug_file, "pc: %04X write to 0x%04X: 0x%02X\n",cpu.pc, a, b);
	switch (ha)
	{
	case 0x0:
	case 0x2:
	...
	case 0xA:
		...
		break;
	case 0xC:
		/*if(ram_compl_mask[(a-0xC000)/32] & (1 << ((a-0xC000)%32)))
			need_clear_ram = 1;*/
		...
		//if(ram_compl_mask[(a>>5)&0xff] & (1 << (a & 0x1F)))
		//	ccmpl_clear_ram();
	...
	case 0xE:
		...
		if ((a & 0xFF00) == 0xFE00)
		{
			...
			if (a < 0xFEA0) LCDP->oam.mem[a & 0xFF] = b;
			break;
		}
		/* return writehi(a & 0xFF, b); */
		if (a >= 0xFF10 && a <= 0xFF3F)
		{
			/*if(oss_enabled)*/ sound_write(a & 0xFF, b); //FIXME (oss_enabled?)
			break;
		}
		if ((a & 0xFF80) == 0xFF80 && a != 0xFFFF)
		{
			ram.hi[a & 0xFF] = b;
			#if USE_HIRAM_CACHE
				hiram_dirty_mask[(a>>5)&0x03] |= (1 << (a & 0x1F));
			#endif
			break;
		}
...
byte mem_read(int a) // int !!
{
	...
	case 0x8:
		if(R_VBK&1) return LCDP->vbank2[a & 0x1FFF];
		/* if ((R_STAT & 0x03) == 0x03) return 0xFF; */

		return LCDP->vbank/*[R_VBK&1]*/[a & 0x1FFF];
	...
	case 0xE:
		...
		if ((a & 0xFF00) == 0xFE00)
		{
			...
			if (a < 0xFEA0) return LCDP->oam.mem[a & 0xFF];
			return 0xFF;
		}
