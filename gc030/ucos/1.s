...
.section .data
	...
	.equ renwuduizhan,	0x20000200 @ 任务堆栈
...
_kongxianrenwu:
	...
	ldr r1, [r0, # 0x14]
	cmp r1, # 0xff                @任务1多少时间执行一次
	bls _kongrenwujixuyunxing     @小于继续运行空任务
	ldr r0, = jiuxubiao
	movs r1, # 1
	str r1, [r0, # 0x04]          @任务1就绪
	bl _diaodu
_kongrenwujixuyunxing:	
	ldr r0, = 0xe000ed04
	ldr r1, = 0x10000000
	str r1, [r0]
	b _kongxianrenwu
_renwu1:
	ldr r2, = renwukongzhikuai
	ldr r0, = jiuxubiao
	movs r1, # 0
	str r1, [r0, # 0x04]           @任务1就绪表清零
	str r1, [r2, # 0x14]           @任务1时间清零
	bl _diaodu
	b _renwu1
