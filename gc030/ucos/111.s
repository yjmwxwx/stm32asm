@ stm32f030f4p6 asm 
@ 温度控制
	 .thumb                 
         .syntax unified
.section .data
	@@！！0X1FFF000内部BOOT里面的函数，只能用于STM32F030F4P6
	.equ fjiesuo,           0x1fffef62 @FPEC解锁
	.equ ojiesuo,           0x1fffef6e @OPTWRE解锁
	.equ quanca,            0x1fffeff8 @全擦除
	.equ yeca,              0x1fffefda @页擦除，每页1K
	.equ zica,              0x1ffff020 @字擦除
	.equ xieflash,          0x1fffef9a @写FLASH
	.equ zixie,             0x1ffff048 @写字节
	.equ xiebaohu,          0x1ffff0a6 @写保护
	.equ jiancedubaohu,     0x1ffff124 @检测读保护
	.equ flashmang,         0x1fffef7a @FLASH忙
        .equ STACKINIT,         0x20001000 @堆栈顶指针@ 1k=1024bit=0x400
	.equ shumaguanma,	0x20000000 @数码管码
	.equ anjianma,		0x20000020 @按键码
	.equ mubiaowendu,	0x20000024 @目标温度
	.equ dangqianwendu,	0x20000028 @当前温度
	.equ dangqiancha,       0x2000002c @PID当前差
	.equ shangcicha,        0x20000030 @PID上次差
	.equ ilishi,		0x20000034 @PID_I的历史
	.equ kp,		0x20000038 @PID参数KP
	.equ ki,		0x2000003c @ PID参数KI
	.equ kd,		0x20000040 @ PID参数KD
	.equ dqrwyxj,           0x20000044 @ 当前任务优先级
	.equ xtzgyxj,           0x20000048 @ 系统里最高优先级
	.equ dangqian,          0x2000004c @  当前TCB任务控制块指针
	.equ dengyunxing,       0x20000050 @ 待运行TCB任务控制块指针
	.equ jiuxubiao,         0x20000054 @ 就绪查找表8个任务
	 @ 保留0X20
	.equ renwubiao,         0x20000074 @ 任务控制块指针表
	@保留0x20
	.equ renwukongzhikuai,  0x20000094 @ 任务控制块起始地址
	@每个控制块两个数据，+0堆栈顶，+4任务时间
	@保留 0x40
	.equ renwudiuzhan,	0x200000d4 @ 任务堆栈
	@每个占用16个32位=0X40，8个就是0x200
	@保留 0X200
	.equ ddd, 0x200002d4
	
	
shumaguanmabiao:                         @数码管码表
	.int 0xfc,0x60,0xda,0xf2,0x66,0xb6,0xbe,0xe0,0xfe,0xf6
shumaguanshuaxinbiao:                    @数码管刷新表
	.int 0xfe00,0xfd00,0xfb00,0xf700,0xef00,0xdf00,0xbf00,0x7f00,0
wendubiao:               @ADC检测到的电压查表法显示温度
	.int 2520,2500,2460,2430,2410,2370,2340,2310,2260,2230,2200,2160,2130,2090,2050,2010,1980,1940,1910,1860,1830,1790,1760,1720,1680,1650,1614,1578,1542,1514,1472,1438,1403,1369,1336,1303,1271,1239,1208,1177,1146,1117,1087,1059,1031,1003,976,949,923,898,873,849,826,803,780,758,737,716,696,676,657,638,620,602,585,568,552,536,521,506,491,477,464,450,437,425,413,401,390,379,368,357,347,337,328,319,310,301,293,284,276,269,251,247,240,234,227,221,215,209,204,198,193,188,183,178,173,169,164,160,156,152,148,144,140,137,133,130,126,123
anjian:              @按键
	.word _caidan1 + 1,_anjian2 + 1, _anjian3 + 1, _adcwendu + 1
pshezhi:             @PID——P设置
	.word _caidan2 +1, _anjian4 +1, _anjian5 +1, _caidan1 +1
ishezhi:             @ I设置
	.word _caidan3 +1, _anjian6 +1, _anjian7 +1, _caidan2 +1
dshezhi:            @ D设置
	.word _baocun +1, _anjian8 +1, _anjian9 +1, _caidan3 +1
.section .text
vectors:          @向量表
        .word STACKINIT              @堆栈指针       
        .word _start + 1             @复位向量
        .word _nmi_handler + 1  
        .word _hard_fault  + 1  
        .word 0 
        .word 0    
        .word 0
	.word 0
	.word 0
	.word 0
	.word 0
	.word _svc + 1
	.word 0
	.word 0
	.word _pendsvzhongduan + 1
	.word _systickzhongduan + 1
	
	

_start:
_shizhong:                     @时钟设置
	ldr r2, = 0x40022000   @FLASH访问控制
	movs r1, # 0x32
	str r1, [r2]
	ldr r0, = 0x40021000 @ rcc
	@0x34时钟控制寄存器 2 (RCC_CR2)
	movs r1, # 0x01
	str r1, [r0, # 0x34]  @ HSI开14M时钟
_dengdai14mshizhongwending:    @等14M时钟稳定
	ldr r1, [r0, # 0x34]
	lsls r1, r1, # 30     @ 左移30位
	bpl _dengdai14mshizhongwending  @ 等待14M时钟稳定

_neicunqingling:
	ldr r0, = 0x20000000
	movs r1, # 0
	ldr r3, = 0x1000
_neicunqinglingxunhuan:
	subs r3, # 4
	str r1, [r0, r3]
	bne _neicunqinglingxunhuan

_chushihuaquanjubianliang:
	ldr r0, = 0x8004000
	ldr r1, = mubiaowendu
	ldr r2, [r0]
	str r2, [r1]
	ldr r2, [r0, # 0x04]
	ldr r1, = kp
	str r2, [r1]
	ldr r2, [r0, # 0x08]
	str r2, [r1, # 0x04]
	ldr r2, [r0, # 0x0c]
	str r2, [r1, # 0x08]
	
_waisheshizhong:	 @ 外设时钟  
	@+0x14=RCC_AHBENR
	@0=DMA @2=SRAM @4=FLITF @6=CRC @17=PA @18=PB @19=PC @20=PD @22=PF
	ldr r0, = 0x40021000
	ldr r1, = 0x460005
	str r1, [r0, # 0x14]
           
	@+0x18外设时钟使能寄存器 (RCC_APB2ENR)
	@0=SYSCFG @5=USART6EN @9=ADC @11=TIM1 @12=SPI1 @14=USART1 @16=TIM15 @17=TIM16 @18=TIM17 @22=DBGMCU
	ldr r1, = 0x200
	str r1, [r0, # 0x18]
	@+0X1C=RCC_APB1ENR
	@1=TIM3 @4=TIM6 @5=TIM7 @8=TIM14 @11=WWDG @14=SPI @17=USRT2 @18=USART3 @20=USART5 @21=I2C1 @22=I2C2 @23=USB @28=PWR
	ldr r1, = 0x100
	str r1, [r0, # 0x1c]
	
_adcchushihua:
	ldr r0, = 0x40012400  @ adc基地址
	ldr r1, = 0x80000000
	str r1, [r0, # 0x08]  @ ADC 控制寄存器 (ADC_CR)  @adc校准
_dengadcjiaozhun:
	ldr r1, [r0, # 0x08]
	 movs r1, r1
	bmi _dengadcjiaozhun   @ 等ADC校准
_kaiadc:
	ldr r1, [r0, # 0x08]
	movs r2, # 0x01
	orrs r1, r1, r2
	str r1, [r0, # 0x08]
_dengdaiadcwending:
	ldr r1, [r0]
	lsls r1, r1, # 31
	bpl _dengdaiadcwending @ 等ADC稳定
_tongdaoxuanze:
	ldr r1, = 0x01
	str r1, [r0, # 0x28]    @ 通道选择寄存器 (ADC_CHSELR)
	ldr r1, = 0x2000        @ 13 连续转换
	str r1, [r0, # 0x0c]    @ 配置寄存器 1 (ADC_CFGR1)
	movs r1, # 0x05         @ 
	str r1, [r0, # 0x14]    @ ADC 采样时间寄存器 (ADC_SMPR)
	ldr r1, [r0, # 0x08]
	movs r2, # 0x04         @ 开始转换
	orrs r1, r1, r2
	str r1, [r0, # 0x08]    @ 控制寄存器 (ADC_CR)


	
_io_she_zhi:
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	@a(0x48000000)b(0x48000400)c(0x48000800)d(0x48000c00)f(0x48001400)
	@ 输入（00），通用输出（01），复用功能（10），模拟（11）
	@偏移0x4 = 端口输出类型 @ （0 推挽），（ 1 开漏）
	@偏移0x8 = 输出速度  00低速， 01中速， 11高速
	@偏移0xC = 上拉下拉 00无上下拉，  01 上拉， 10下拉
	@偏移0x10 = 输入数据寄存器
	@偏移0x14 = 输出数据寄存器
	@偏移0x18 = 端口开  0-15置位 
	@偏移0x28 = 端口关
	@0X20 = 复用低
	@GPIO口0（0-3位）每个IO口占用4位
	@ AF0 = 0X0000, AF1 = 0X0001, AF2 = 0X0010 AF3 = 0X0011, AF4 = 0X0100
	@ AF5 = 0X0101, AF6 = 0X0111, AF7 = 0X1000
	@0x24 = 复用高
	@GPIO口8 （0-3位）每个IO口占用4位
	@ AF0 = 0X0000, AF1 = 0X0001, AF2 = 0X0010 AF3 = 0X0011, AF4 = 0X0100
	@ AF5 = 0X0101, AF6 = 0X0111, AF7 = 0X1000
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	ldr r0, = 0x48000000
	ldr r1, = 0x28005403
	str r1, [r0]

	ldr r0, = 0x48000400
	movs r1, # 0x08
	str r1, [r0]
	
	
	ldr r0, = 0x48001400
	movs r1, # 0
	str r1, [r0]
	movs r1, # 0x05
	str r1, [r0, # 0x0c]

_tim14chushihua:
	ldr r0, = 0x40002000
	ldr r1, = 16000
	str r1, [r0, # 0x28]  @psc
	ldr r1, = 5000
	str r1, [r0, # 0x2c]  @arr
	ldr r1, = 0x60
	str r1, [r0, # 0x18]
	ldr r1, = 0x01
	str r1, [r0, # 0x20]
	ldr r1, = 0
	str r1, [r0, # 0x34]
	ldr r1, = 0x81
	str r1, [r0]

_renwuchushihua:                     @任务初始化
	ldr r0, = dengyunxing        @等运行任务控制块指针
	ldr r1, = renwubiao
	str r1, [r0]

	ldr r0, = jiuxubiao   	     @就绪表
	movs r1, # 0x01
	str r1, [r0]                 @任务0
	str r1, [r0, # 0x04]         @任务1

_chuangjianrenwu0：                  @创建任务0
	ldr r0, = renwubiao          @任务表
	ldr r1, = renwuduizhan       @任务0堆栈
	str r1, [r0]
	ldr r2, = 0xffff             @任务减到0所需要的时间
	str r2, [r0, # 0x04]
_chuangjianrenwu1:	             @创建任务1
	adds r1, r1, # 0x40          @任务1堆栈
	str r1, [r0, # 0x08]
	ldr r2, = 0xff
	str r2, [r0, # 0x0c]
	
_renwudiaodu:	  @打开任务自动调度
_systick:
	@ 0xe000ed20 30-31 优先级
	ldr r0, = 0xe000e010
	ldr r1, = 1000 @ 重载数
	str r1, [r0, # 4] @ 写入重载数
	str r1, [r0, # 8] @ 清除当前数
	movs r1, # 0X07
	str r1, [r0] @ 开定时器

	ldr r0, = 0xe000ed04
	ldr r1, = 0x10000000
	str r1, [r0]    @ 触发中断pendsv
	
_tingxia1:
	b tingxia1
	
_renwu0:
	ldr r0, = 0x01234567
	mov r12, r0

	ldr r0, = renwubiao
	ldr r2, = 0xffff
	str r2, [r0, # 0x04]

	ldr r0, = jiuxubiao
	movs r1, # 0
	str r1, [r0]         @ 放开任务

	ldr r0, = 0xe000ed04
	ldr r1, = 0x10000000
	str r1, [r0]         @触发中断
	b _renwu0
_renwu1:
	ldr r0, = 0x55555555
	mov r12, r0

	ldr r0, = renwubiao
	ldr r2, = 0xff
	str r2, [r0, # 0x0c]

	ldr r0, = jiuxubiao
	movs r1, # 0
	str r1, [r0, # 0x04]     @任务2

	ldr r0, = 0xe000ed04
	ldr r1, = 0x10000000
	str r1, [r0]             @触发中断
	b _renwu1
	

	
_adcwendu:
	ldr r7, = 0xff
	movs r2, # 0
_adcpingjun:
	ldr r0, = 0x40012440
	ldr r1, [r0]
	adds r2, r2, r1
	subs r7, # 1
	bne _adcpingjun
	lsrs r0, r2, # 8
	ldr r1, = 3300
	muls r0, r1
	ldr r1, = 4096
	bl _chufa
	ldr r2, = wendubiao
	movs r4, # 0
_panduanwendu:
	ldr r3, [r2, r4]
	cmp r3, r0
	bls _dedaowendu
	adds r4, r4, # 4
	b _panduanwendu
_dedaowendu:
	lsrs r4, # 2
	ldr r0, = dangqianwendu
	str r4, [r0]
	ldr r0, = mubiaowendu
	ldr r1, [r0]
	ldr r2, = 100000
	muls r2, r1
	adds r2, r4
	mov r0, r2
	ldr r1, = 8
	ldr r2, = shumaguanma
	bl _zhuanshumaguanma
	movs r0, # 8
	lsls r0, # 2
	bl _xieshumaguan
	bl _pid
        ldr r0, = 0x48001410
	ldr r1, [r0]
	cmp r1, # 0
	bne _tiaoguoanjian1_1
	ldr r5, = 0x55555
_anjian1yanshi1:	
	subs r5, # 1
	bne _anjian1yanshi1
_tiaoguoanjian1_1:	
	ldr r2, = anjian
	ldr r0, [r2, # 0x0c]
	mov lr, r0
	lsls r1, # 2
	ldr r0, [r2, r1]
	mov pc, r0
	
_caidan1:
	ldr r2, = kp
	ldr r1, [r2]
	ldr r7, = pshezhi
	b _candanxunhuan
_caidan2:
	ldr r2, = ki
	ldr r7, = ishezhi
	b _candanxunhuan
_caidan3:
	ldr r2, = kd
	ldr r7, = dshezhi
	b _candanxunhuan
_baocun:
	b _xieflash
_candanxunhuan:	 @ r0 @ 
	ldr r0, [r2]
	ldr r1, = 8
	ldr r2, = shumaguanma
	bl _zhuanshumaguanma
	movs r0, # 8
	lsls r0, # 2
	bl _xieshumaguan
        ldr r0, = 0x48001410
        ldr r1, [r0]
	cmp r1, # 0
	bne _tiaoguoanjian1
	ldr r5, = 0x55555
_anjian1yanshi:
	subs r5, # 1
	bne _anjian1yanshi
	
_tiaoguoanjian1:	
        ldr r0, [r7, # 0x0c]
        mov lr, r0
	lsls r1, # 2
        ldr r0, [r7, r1]
        mov pc, r0
	
_anjian2:
	push {r0-r1}
	ldr r0, = mubiaowendu
	ldr r1, [r0]
	adds r1, # 1
	cmp r1, # 120
	bls _anjianfanhui
	movs r1, # 120
_anjianfanhui:	
	str r1, [r0]
	pop {r0-r1}
	bx lr
_anjian3:
	push {r0-r1}
	ldr r0, = mubiaowendu
        ldr r1, [r0]
        subs r1, # 1
        cmp r1, # 120
        bls _anjianfanhui1
        movs r1, # 0
_anjianfanhui1:
        str r1, [r0]
	pop {r0-r1}
	bx lr

_anjian4:
	push {r0-r2}
	ldr r0, = kp
	ldr r1, [r0]
	adds r1, # 1
	ldr r2, = 5000
	cmp r1, r2
	bls _anjianfanhui4
	ldr r1, = 5000
_anjianfanhui4:
	str r1, [r0]
	pop {r0-r2}
	bx lr
_anjian5:
	push {r0-r2}
	ldr r0, = kp
	ldr r1, [r0]
	subs r1, # 1
	ldr r2, = 5000
	cmp r1, r2
	bls _anjianfanhui5
	movs r1, # 0
_anjianfanhui5:
	str r1, [r0]
	pop {r0-r2}
	bx lr

_anjian6:
	push {r0-r2}
	ldr r0, = ki
	ldr r1, [r0]
	adds r1, # 1
	ldr r2, = 5000
	cmp r1, r2
	bls _anjianfanhui6
	ldr r1, = 5000
_anjianfanhui6:
	str r1, [r0]
	pop {r0-r2}
	bx lr
_anjian7:
	push {r0-r2}
	ldr r0, = ki
	ldr r1, [r0]
	subs r1, # 1
	ldr r2, = 5000
	cmp r1, r2
	bls _anjianfanhui7
	movs r1, # 0
_anjianfanhui7:
	str r1, [r0]
	pop {r0-r2}
	bx lr

_anjian8:
	push {r0-r2}
	ldr r0, = kd
	ldr r1, [r0]
	adds r1, # 1
	ldr r2, = 5000
	cmp r1, r2
	bls _anjianfanhui8
	ldr r2, = 5000
_anjianfanhui8:
	str r1, [r0]
	pop {r0-r2}
	bx lr
_anjian9:
	push {r0-r2}
	ldr r0, = kd
	ldr r1, [r0]
	subs r1, # 1
	ldr r2, = 5000
	cmp r1, r2
	bls _anjianfanhui9
	movs r1, # 0
_anjianfanhui9:
	str r1, [r0]
	pop {r0-r2}
	bx lr

	
	
_pid:
	push {r0-r7}
	ldr r0, = mubiaowendu 
	ldr r1, [r0]
	ldr r2, [r0, # 0x04]
	subs r1, r1, r2
	ldr r2, [r0, # 0x08]
	str r2, [r0, # 0x0c]    @ -1
	str r1, [r0, # 0x08]    @ 0
_ppp:
	ldr r3, = kp
	ldr r3, [r3]
	mov r4, r1
	muls r4, r4, r3
_iii:
	ldr r3, = ki
	ldr r3, [r3]
	mov r5, r1
	muls r5, r5, r3
	ldr r3, [r0, # 0x10]
	adds r5, r5, r3
	movs r3, # 0
	ldr r6, = 500
	subs r3, r3, r6
	cmp r5, r3
	bhi _cunchuiii
	mov r5, r3
_cunchuiii:
	cmp r5, r6
	bls _chucuniii1
	movs r5, r6
_chucuniii1:	
	str r5, [r0, # 0x10]

_ddd:
	ldr r3, = kd
	ldr r3, [r3]
	subs r1, r1, r2
	muls r3, r1
	
	adds r4, r4, r5
	adds r4, r4, r3
	bpl _buxiaoyu0
	movs r4, # 0
_buxiaoyu0:	
	ldr r0, = 0x40002034 @ tim14cr1
	str r4, [r0]
	pop {r0-r7}
	bx lr

_xieflash:
	cpsid i  @关中断
       @ FPEC解锁
	ldr r4, = fjiesuo
	movs r5, # 3
	add r5, pc
	mov lr, r5
	mov pc, r4

	@ OPTWRE解锁
	ldr r4, = ojiesuo
	movs r5, # 3
	add r5, pc
	mov lr, r5
	mov pc, r4
	@ 擦除
	ldr r4, = yeca
	ldr r0, = 0x8004000
	movs r5, # 3
	add r5, pc
	mov lr, r5
	mov pc, r4

	ldr r4, = xieflash
	ldr r0, = 0x8004000
	ldr r5, = mubiaowendu
	ldr r1, [r5]
	movs r5, # 3
	add r5, pc
	mov lr, r5
	mov pc, r4

	ldr r4, = xieflash
	ldr r0, = 0x8004004
	ldr r5, = kp
	ldr r1, [r5]
	movs r5, # 3
	add r5, pc
	mov lr, r5
	mov pc, r4

        ldr r4, = xieflash
	ldr r0, = 0x8004008
	ldr r5, = ki
	ldr r1, [r5]
	movs r5, # 3
	add r5, pc
	mov lr, r5
	mov pc, r4
	
	ldr r4, = xieflash
	ldr r0, = 0x800400c
	ldr r5, = kd
	ldr r1, [r5]
	movs r5, # 3
	add r5, pc
	mov lr, r5
	mov pc, r4
	
	ldr r0, = 0xe000ed0c
	ldr r1, = 0x05fa0004
	str r1, [r0] 		@复位

_tingxia:
	b _tingxia
	
	
_zhuanshumaguanma:@ 16进制转数码管码
		@ R0要转的数据， R1长度，R2结果表首地址
	push {r3-r7,lr}
	ldr r7, = shumaguanmabiao
	mov r5, r0
	mov r6, r1
	movs r1, # 10
_xunhuanqiuma:
	bl _chufa
	mov r4, r0
	muls r4, r1
	subs r3, r5, r4
	lsls r3, # 2
	ldr r4, [r7, r3]
	str r4, [r2]
	mov r5, r0
	adds r2, r2, # 4
	subs r6, # 1
	bne _xunhuanqiuma
	pop {r3-r7,pc}
_xieshumaguan: @ r0=位数 @数码管码
	push {r2-r7,lr}
	movs r7, # 0
	mov r6, r0
	ldr r5, = shumaguanma
	ldr r2, = shumaguanshuaxinbiao
_shumaguanshuaxin:
	ldr r3, [r5, r7]
	ldr r4, [r2, r7]
	orrs r4, r4, r3
	mov r0, r4
	bl _xie595
	adds  r7, r7, # 4
	cmp r7, r6
	bls _shumaguanshuaxin
	pop {r2-r7,pc}
	
_xie595: @ R0=要写的数据
	push {r0-r7,lr}
	mov r5, r0
	movs r0, # 0x40
	movs r7, # 0x80 
	movs r6, # 0x20 
	movs r3, # 16   @16位
	ldr r2, = 0x48000000 @ gpioa
	movs r1, # 0x01
_xunhuan595:
	str r7, [r2, # 0x18]
	bl _yanshi
	tst r5, r1
	bne _xie595_0
	str r0, [r2, # 0x28]
	b _suocun595
_xie595_0:
	str r0, [r2, # 0x18]
_suocun595:
	bl _yanshi
	lsrs r5, # 1
	str r7, [r2, # 0x28]
	bl _yanshi
	subs r3, r3, # 1
	bne _xunhuan595
	str r6, [r2, # 0x18]
	bl _yanshi
	str r6, [r2, # 0x28]
	bl _yanshi
	pop {r0-r7,pc}

_yanshi:
	push {r7}
	ldr r7, = 30
_yanshi11:
	subs r7, # 1
	bne _yanshi11
	pop {r7}
	bx lr
	
_chufa:	@软件除法
	@ r0 除以 r1 等于 商(r0)不要余数
	push {r1-r4}
	mov r2, r0
	movs r3, # 1
	lsls r3, r3, # 31
	movs r0, # 0
	mov r4, r0
_chufaxunhuan:       
	lsls r2, r2, # 1
	adcs r4, r4, r4
	cmp r4, r1
	bcc _chufaweishubudao0
	adds r0, r0, r3
	subs r4, r4, r1
_chufaweishubudao0:
	lsrs r3, r3, # 1
	bne _chufaxunhuan
	pop {r1-r4}
	bx lr
	
	
_nmi_handler:
	bx lr
_hard_fault:
	ldr r0, = mubiaowendu
	movs r1, # 0
	str r1, [r0]
	ldr r0, = kp
	str r1, [r0]
	str r1, [r0, # 0x04]
	str r1, [r0, # 0x08]
	bl _xieflash
_svc:
	bx lr
_pendsvzhongduan:             @PENDSV中断
	cpsid i               @ 关中断
	mrs r0, psp           @ 读PSP当前地址到R0
	cmp r0, # 0	      @ psp等不等于0	
	bne _qiehuanrenwu     @ 不等于0就切换任务
	subs r0, r0, # 0X20   @ PSP堆栈保留8个32位数
	stmia r0!, {r4-r7}    @保存R4-R7到上个任务的堆栈
_qiehuanrenwu:                @ 切换任务
	ldr r0, = dqrwyxj     @ 当前任务优先级
	ldr r1, = xtzgyxj     @ 系统最高优先级
	ldr r2, [r1]          @ 最高优先级读到R2
	str r2, [r0]          @ 把最高优先级的数写入当前优先级
	ldr r0, = dangqian    @ 当前任务TCB控制块指针
	ldr r1, = dengyunxing @ 等待运行TCB任务控制块指针
	ldr r2, [r1]          @ 取出待运行任务控制块所在地址
	str r2, [r0]          @ 待运行控制块地址写到当前任务控制块指针
	ldr r0, [r2]          @ 取出待运行任务堆栈地址到R0
	ldmia r1!, {r4-r7}    @ 当前任务堆栈里的数据写到R4-R11
	adds r0, r0, # 0x20   @ 当前任务堆栈顶
	msr psp, r0
	movs r0, # 0x04
	mov r1, lr
	orrs r1, r1, r0           @ LR位2等于1， 返回后使用PSP堆栈
	mov lr, r1
	cpsie i               @ 开中断
	bx lr
_systickzhongduan:
	ldr r2, = jiuxubiao
	ldr r0, = renwukongzhikuai  @任务控制块地址
	ldr r1, [r0, # 0x04]
	subs r1, r1, # 1
	bmi _tiaoguoshijian                @ 跳过任务1的时间
	str r1, [r0, # 0x04]
	movs r1, # 1
	str r1, [r2]                @如果时间小于0任务1就绪
_tiaoguoshijian:
	ldr r1, [r0, # 0x0c]
	subs r1, r1, # 1
	bmi _tiaoguoshijian1
	str r1, [r0, # 0x0c]
	movs r1, # 1
	str r1, [r2, # 0x04]         @任务1就绪
_tiaoguoshijian1:
	bx lr
